...
- Can turn on and off the option for the ai to shoot the player/object (bullet goes to its last location, doesn't follow it if it moves) Note: this is bugged*/
public class PatrollingAlgorithms : MonoBehaviour
{
    ...
    public bool firstJumpSetLocation; //Set the location of the AI for starting the platforms (stop it from falling at the top of the screen to the bottom)
    ...
    void Start()
    {
        ...
        {
            transform.position = jumpStartingLocation.transform.position;
        }
    ...
    {
        //Start at bottom of screen not top
        if(firstJumpSetLocation)
        {
            transform.position = jumpStartingLocation.transform.position;
            firstJumpSetLocation = false;
        }
